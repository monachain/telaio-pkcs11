...
/**
 * package name changed by lgcns to compile
 */
package com.lgcns.blockchain.pkcs11;
...
import com.lgcns.blockchain.pkcs11.Secmod.*;
...
import static com.lgcns.blockchain.pkcs11.P11Util.*;
import static com.lgcns.blockchain.pkcs11.TemplateManager.O_IMPORT;
...
final class P11KeyStore extends KeyStoreSpi {
    ...
    private static String isStorePublicKey;
    ...
    private void updatePkey(String alias,
        ...
        try {
            ...
            if (isHexDecimal(alias)) {
                attrs = new CK_ATTRIBUTE[] {
                        new CK_ATTRIBUTE(CKA_LABEL, alias),
                        new CK_ATTRIBUTE(CKA_ID, hexStringToByteArray(alias)) };
            }
            else {
                attrs = new CK_ATTRIBUTE[] {
                        new CK_ATTRIBUTE(CKA_LABEL, alias),
...
//                };
    ...
    private void storeCert(String alias, X509Certificate cert)
        ...
        if (alias != null) {
            ...
            // by lgcns            
            if (isHexDecimal(alias)) {
                attrList.add(new CK_ATTRIBUTE(CKA_ID, hexStringToByteArray(alias)));
            }
            else {
...
//            attrList.add(new CK_ATTRIBUTE(CKA_ID, alias));
        ...
        isStorePublicKey = System.getProperty("com.lgcns.blockchain.pkcs11.isStorePublicKey", "false");
        if ("true".equalsIgnoreCase(isStorePublicKey)) storePublicKey (alias, cert);
    }
    ...
    private void storePublicKey(String alias, X509Certificate cert) {
    	PublicKey pubKey = cert.getPublicKey();
   	
    	if (pubKey instanceof ECPublicKey) {
    		ECPublicKey ecPubKey = (ECPublicKey)pubKey;
    		ECParameterSpec params = ecPubKey.getParams();
            ...
            CK_ATTRIBUTE[] idAttrs = null;
            // by lgcns
            if (isHexDecimal(alias)) {
            	idAttrs = new CK_ATTRIBUTE[] {
            		new CK_ATTRIBUTE(CKA_LABEL, alias),
            		new CK_ATTRIBUTE(CKA_ID, hexStringToByteArray(alias)),
            	};
            }
            ...
            attrs = addAttribute(attrs, idAttrs[0]);
            attrs = addAttribute(attrs, idAttrs[1]);

            try {
                attrs = token.getAttributes
                        (TemplateManager.O_IMPORT, CKO_PUBLIC_KEY, CKK_EC, attrs);
            ...
            try {
                ...
                long keyID = token.p11.C_CreateObject(session.id(), attrs);
            } catch (PKCS11Exception e) {
				e.printStackTrace();
			} finally {
                token.releaseSession(session);
    ...
    private void storePkey(String alias, KeyStore.PrivateKeyEntry pke)
        ...
        if (key instanceof RSAPrivateKey) {
            ...
            if (idAttrs[0] == null) {
                if (isHexDecimal(alias)) {
                    idAttrs[0] = new CK_ATTRIBUTE(CKA_LABEL, alias);
                    idAttrs[1] = new CK_ATTRIBUTE(CKA_ID, hexStringToByteArray(alias));
                }
                else {
                    idAttrs[0] = new CK_ATTRIBUTE(CKA_LABEL, alias);
                    idAttrs[1] = new CK_ATTRIBUTE(CKA_ID, alias);
                }
            }
//            if (idAttrs[0] == null) {
...
//            }
    ...
    private ArrayList<AliasInfo> mapPrivateKeys(ArrayList<byte[]> pkeyIDs,
        ...
        for (byte[] pkeyID : pkeyIDs) {
            ...
            if (!foundMatch) {
            	/**
            	 * updated by lgcns for private without cert
            	 */
            	if (debug != null) {
            		debug.println("Set Private Key without cert.::" + getID(pkeyID));
            	}
            	
				AliasInfo aliasInfo = new AliasInfo(getID(pkeyID), pkeyID, true, null);
				aliasInfo.matched = true;
				
				aliasMap.put(getID(pkeyID), aliasInfo);

				/*
				 * 
				 * if (debug != null) { debug.println
				 * ("did not find match for private key with CKA_ID [" + getID(pkeyID) +
				 * "] (ignoring entry)"); }
				 */
            }
        }
    ...
    // by lgcns
	private boolean isHexDecimal(String cadena) {
	    if ( cadena.length() == 0 || 
	    ...
	    if (cadena.length() % 2 != 0) return false;
